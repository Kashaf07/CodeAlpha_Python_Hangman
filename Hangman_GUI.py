...
while run:
    ...
    for i in range(0,len(selected_word)):
        x += 60
        exec('d{}=Label(root,text="_",bg="#E7FFFF",font=("times new roman",40))'.format(i))
    ...
    ex.place(x=770,y=10)
    s2 = 'SCORE:'+str(score)
    s1 = Label(root,text = s2,bg = "#E7FFFF",font = ("times new roman",25))
    s1.place(x = 10,y = 10)
    ...
    def check(letter,button):
        ...
        if letter in selected_word:
            ...
            if win_count == len(selected_word):
                score += 1
                answer = messagebox.askyesno("Congratulations!", f"You guessed the word: {selected_word} \nWANT TO PLAY AGAIN?")
                if answer == True:
                    run = True
                    root.destroy()   
                else:
            ...
            exec('c{}.place(x={},y={})'.format(count+1,400, 100))
            if count == 6:
                answer = messagebox.askyesno("Game Over", f"You ran out of attempts. The word was: {selected_word}\nWANT TO PLAY AGAIN?")
                if answer == True:
                    run = True
                    score = 0
                    root.destroy()
